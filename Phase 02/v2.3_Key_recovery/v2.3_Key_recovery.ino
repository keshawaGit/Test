...
boolean Tx_start=true;
boolean csvEnd =false;
int Tx_count =-1;
boolean isKeyRecovered =false;
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (String(topic) == "esp32/output") {
   ...
   if(messageTemp == "bv3ON"){
    ...
    if (messageTemp == "keyMatched"){
      bus3_key();
      isKeyRecovered =true;
      
      }
    else if(messageTemp == "keyUnmatched"){
      isKeyRecovered = false;       
      }          
    ...
    client.loop();
      ...
      while(bus3_power() > 20){
          if(turns ==0){
                  Tx_count++;
                  char s_log[] ="start";
                  if (client.publish("esp32/bus3/start", s_log) == true) {
                      Serial.println("Success start log");
                  } else {
                      Serial.print("Error start log");
                      Serial.println(client.state());
                  } 
            }
          Tx_start =true;
          //get average of bus power to array list     
          bus3_data();
          csvEnd =false;
...
float bus3_power(){
  ...
  power_temp = ina226_2.getBusPower();
  //Serial.println(power_temp);
  return power_temp;
  }
...
void bus3_key(){
      ...
      char cstr[16];
      itoa(num, cstr, 10);
      if (client.publish("esp32/bus3/count", cstr) == true) {
      Serial.println("Success key count");
    } else {
      Serial.println("Error key count");
    }  
  }
