...
boolean bv1Request = false;
boolean lv1Request = false;
boolean cur1Request =false;
boolean pow1Request = false;
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (String(topic) == "esp32/output") {
    ...
    if(messageTemp == "checkON"){
      isCheck = true;
    }
    else if(messageTemp == "checkOFF"){
      isCheck = false;
    }   
...
void checkTel(){
  StaticJsonDocument<96> tel_;

  tel_["start_status"] = isStart; 
  tel_["dev_status"] = isDevice; 
  tel_["check_status"] = isCheck; 

  char tel_output[256];
  serializeJson(tel_,tel_output);
  //Serial.println("Sending message to ESP32/Status");
  Serial.println(tel_output);
 
  if (client.publish("esp32/TEL", tel_output) == true) {
    //Serial.println("Success status");
  } else {
    Serial.println("Error status");
  }
